...
export function AiPanel({
    ...
    loopRef.current = new AgentLoop({
      ...
      events: {
        ...
        onError: (error) => {
          ...
          void Promise.all([window.slidesApi.getAiSettings(), window.slidesApi.aiGskStatus()])
            .then(([current, status]) => {
  ...
  const newChat = () => {
    ...
    inputRef.current?.focus()
    void projectApi()
      ?.newChat({ filePath: currentFilePath ?? null, tempChatId: `unsaved-${Date.now()}` })
      .then((ids) => {
        chatRefIds.current = ids
      })
      .catch(() => {
  ...
  /** Open the picker on this deck's stored conversations, newest first. */
  const openSessions = () => {
    setSessionsOpen((open) => !open)
    void projectApi()
      ?.listChatsForFile({ filePath: currentFilePath ?? null })
  ...
  const loadSession = (chatId: string) => {
    ...
    setChat([])
    void api
      .switchChat({ filePath: currentFilePath ?? null, chatId })
